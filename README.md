...
- **Defense:** the bot **never** pays to clear an audit automatically. Once one of your citizens is audited, catching it up is a deliberate, user-initiated action — use the **Pay to current** button on the token row. It can still pay *proactively* so your citizens never become auditable in the first place, and prepay up to 7 epochs to lock the current (lower) rate. The only automatic response to an audit is spending an already-held bribe, and only if you opt in (`autoUseBribe`, off by default) — a bribe is free but consumed and leaves the citizen still delinquent. Any citizen you **uncheck** in the JIT panel is excluded from *every* automatic payment.
