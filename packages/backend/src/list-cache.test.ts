...
const { loadAllyTokens, loadBigBoys, invalidateListCache } = await import("./runtime.js");

/**
 * The curated lists are cached in memory and invalidated on the file's mtime+size, because
 * they sit on the hot path: fetchOffenseCandidates reads the ally list and readTargets
...
describe("list cache: serves repeat reads without re-parsing, but never goes stale", () => {
  ...
  it("caches the DERIVED big-boys shape, not just the parse", () => {
    fs.writeFileSync(
      nodePath.join(tmpDir, "big-boys.json"),
      JSON.stringify({ owners: { Graveyard: ["4335", "909"], Hedo: ["1575"] } }),
    );
    const first = loadBigBoys();
    expect(first.tokenIds).toEqual(["4335", "909", "1575"]);
    // Same object identity => the normalize + dedupe was memoized, not redone.
    expect(loadBigBoys()).toBe(first);
  });

  it("de-duplicates a big-boys id listed under two operators", () => {
    fs.writeFileSync(
      nodePath.join(tmpDir, "big-boys.json"),
      JSON.stringify({ owners: { A: ["4335", "909"], B: ["4335"] } }),
    );
    expect(loadBigBoys().tokenIds).toEqual(["4335", "909"]);
  });

  it("re-derives big-boys after the file changes", () => {
    const p = nodePath.join(tmpDir, "big-boys.json");
    fs.writeFileSync(p, JSON.stringify({ owners: { A: ["4335"] } }));
    expect(loadBigBoys().tokenIds).toEqual(["4335"]);
    fs.writeFileSync(p, JSON.stringify({ owners: { A: ["4335"], B: ["711", "796"] } }));
    expect(loadBigBoys().tokenIds).toEqual(["4335", "711", "796"]);
  });

  it("survives a big-boys file with no owners key", () => {
    fs.writeFileSync(nodePath.join(tmpDir, "big-boys.json"), JSON.stringify({}));
    expect(loadBigBoys()).toEqual({ tokenIds: [], owners: {} });
  });
});
