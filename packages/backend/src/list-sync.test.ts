...
beforeEach(() => {
  ...
  remote = {
    ...
    [BIGBOYS]: JSON.stringify({ owners: { Graveyard: ["4335", "909"] } }),
  };
...
describe("syncDefaultLists: refreshing shipped lists", () => {
  ...
  it("creates a list that the carried-over data/ folder never had", async () => {
    ...
    expect(JSON.parse(read(BIGBOYS))).toEqual({ owners: { Graveyard: ["4335", "909"] } });
  });
  ...
  it("records hashes so the next run can tell a local edit from its own write", async () => {
    ...
    expect(Object.keys(manifest).sort()).toEqual([ALLIES, BIGBOYS, SKIPPERS, TARGETS].sort());
  });
});
...
describe("syncDefaultLists: a bad payload never disarms a safety list", () => {
  ...
  // engine audit teammates, and an empty big-boy roster loses the attribution tags. So an
  ...
  it("rejects a big-boys payload with no ids in any owner group", async () => {
    write(BIGBOYS, JSON.stringify({ owners: { Graveyard: ["4335"] } }, null, 2) + "\n");
    remote[BIGBOYS] = JSON.stringify({ owners: { Graveyard: [] } });
    await syncDefaultLists();
    expect(JSON.parse(read(BIGBOYS))).toEqual({ owners: { Graveyard: ["4335"] } });
  });
  ...
  it("rejects an array where the owner-map shape is required", async () => {
    write(BIGBOYS, JSON.stringify({ owners: { Graveyard: ["4335"] } }, null, 2) + "\n");
    remote[BIGBOYS] = JSON.stringify(["4335"]);
    await syncDefaultLists();
    expect(JSON.parse(read(BIGBOYS))).toEqual({ owners: { Graveyard: ["4335"] } });
  });
